...
const HeaderCard = ({info}) => {
  return (
    <Box  className={`p-6 mx-1 md:m-0  dark:bg-black text-white md:w-[368px] ${info.id ===1&& 'bg-[#43baf6ea]'} ${info.id ===2&& 'bg-[#6687ffef]'} ${info.id ===3&& 'bg-[#9878fff5]'}`}>
        <p className='uppercase text-[13px] font-light'>{info.heading}</p>
        <h3 className='uppercase text-xl pb-4 font-bold'>{info.title}</h3>
